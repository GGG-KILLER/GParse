...
namespace GParse.Verbose.Optimization
{
    public class MatchTreeOptimizer : IMatcherTreeVisitor<BaseMatcher>
    {
        ...
        private BaseMatcher[] JoinIntersectingRanges ( BaseMatcher[] array )
        {
            ...
            var ranges = new Stack<(Range, Int32)> ( array.Length );
            ...
            {
                (Range range1, Int32 idx) = ranges.Pop ( ); (Range range2, _) = ranges.Peek ( );
                if ( range1.IntersectsWith ( range2 ) || range1.IsNeighbourOf ( range2 ) )
                {
                    ranges.Pop ( );
                    ranges.Push ( (new Range ( Math.Min ( range1.Start, range2.Start ), Math.Max ( range1.End, range2.End ) ), idx) );
                }
                else
